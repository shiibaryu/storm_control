...
static unsigned int total_cpu_bit(unsigned int bps)
{
	int cpu=0;
...
static void pps_threshold_check(void){
	if(sc_dev.pblc->pps_counter >= threshold && (sc_dev.d_flag & FLAG_DOWN)){
    ...
    else if(sc_dev.pblc->pps_counter < threshold && (sc_dev.d_flag & FLAG_DOWN)){
		sc_dev.pblc->pps_counter = 0;
		initialize_pps_counter(pc_packet);
		mod_timer(&sc_timer, jiffies + TIMER_TIMEOUT_SECS*HZ);
    ...
    else if(sc_dev.pblc->pps_counter >= low_threshold && (sc_dev.d_flag & FLAG_UP)){
		sc_dev.pblc->pps_counter = 0;
	    	initialize_pps_counter(pc_packet);
		mod_timer(&sc_timer, jiffies + TIMER_TIMEOUT_SECS*HZ);
...
static void bps_threshold_check(void){
	if(sc_dev.pblc->bps_counter >= threshold && (sc_dev.d_flag & FLAG_DOWN)){
		sc_dev.d_flag = FLAG_UP;
    ...
    else if(sc_dev.pblc->bps_counter < threshold && (sc_dev.d_flag & FLAG_DOWN)){
		sc_dev.pblc->bps_counter = 0;
		initialize_bps_counter(pc_packet);
		mod_timer(&sc_timer, jiffies + TIMER_TIMEOUT_SECS*HZ);
	    	printk(KERN_INFO "Packet pakcet per second was less than the threthold.\n");
    ...
    else if(sc_dev.pblc->bps_counter >= low_threshold && (sc_dev.d_flag & FLAG_UP)){
		sc_dev.pblc->bps_counter = 0;
	    	initialize_bps_counter(pc_packet);
		mod_timer(&sc_timer, jiffies + TIMER_TIMEOUT_SECS*HZ);
	    	printk(KERN_INFO "Packet pakcet per second was more than the lowthrethold.\n");
